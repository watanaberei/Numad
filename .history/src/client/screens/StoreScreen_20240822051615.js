...
window.storeActions = {
  ...
  toggleSaveStore: async function(storeName, storeId) {
    ...
    try {
      const response = await fetch('/api/save-store', {
        ...
        body: JSON.stringify({ storeId, action: saveButton.classList.contains('saved') ? 'unsave' : 'save' })
      });
  ...
  toggleCheckInStore: async function(storeName, storeId) {
    ...
    try {
      const response = await fetch('/api/check-in', {
        ...
        body: JSON.stringify({ storeId, action: checkinButton.classList.contains('checked-in') ? 'checkout' : 'checkin' })
      });
      const data = await response.json();
  ...
  showRatingElement: function() {
    const ratingElement = document.getElementById('store-rating');
    if (ratingElement) {
      ratingElement.style.display = 'block';
    }
  },

  hideRatingElement: function() {
    const ratingElement = document.getElementById('store-rating');
    if (ratingElement) {
  ...
  rateStore: async function(rating, storeId) {
    try {
      const response = await fetch('/api/rate-store', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
