...
const socket = io('http://localhost:4000');
    ...
    socket.on('impression_update', (data) => {
      updateImpressionUI(data.storeId, data.likes, data.dislikes);
    });
// const socket = io('http://localhost:4000');
const StoreScreen = {
  render: async () => {
  ...
  <div class="item">
    ...
    const impressionButton = document.getElementById('impression-button');
    impressionButton.addEventListener('click', async () => {
      const impressionData = {
        // Add relevant impression data here
        timestamp: new Date().toISOString(),
        // ... other impression details
      };
      await StoreScreen.handleImpression(impressionData);
    });
    handleImpression: async (impressionData) => {
      try {
        await sendImpression(impressionData);
        // Handle successful impression (e.g., update UI, show confirmation)
      } catch (error) {
...
impressionButtons.forEach(button => {
  button.addEventListener('click', async () => {
    ...
    try {
      ...
      updateImpressionUI(storeId, response.likes, response.dislikes);
    } catch (error) {
      console.error('Error sending impression:', error);
    }
  });
