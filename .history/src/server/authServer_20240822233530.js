...
// Add this new route for handling impressions
app.post('/api/impression', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Add the impression to the user's impressions array
    // You might need to adjust this based on your User schema
    user.impressions.push(impressionData);
    await user.save();

    res.status(200).json({ message: 'Impression added successfully' });
  } catch (error) {
